...
def paypal_login(context, label=None, css_class=None, icon_class=None,
                 only_login=False, error_message=None, error_class=None):
    """
    Renders a 'Sign in with PayPal' button.
    ...
    Parameters:
        ...
        - only_login: if True the button will only try to log the user in
            without registering it, if an user that matches the PayPal
    ...
    context['action'] = reverse('socialnetworks:paypal:login')
    ...
    return template.loader.render_to_string(
        'paypal/login_button.html', context)
